...
class TestViews(TestCase):
    ...
    def test_login(self):
        with self.client as c:
            ...
            def test_login(user, pwd, check=[]):
                ...
                for phrase in check:
                    self.assertTrue(
                        phrase.lower() in str(res.data).lower(), 'Result should contain %s' % phrase)

            res = c.get('/login')
            self.assertTrue(
                'User Name:' in str(res.data) and 'Password:' in str(res.data))
            ...
            def test_login_json(user, pwd, success=True, failure=False, email=None):
                ...
                if success:
                    self.assert200(res)
                    self.assertTrue(
                        res.json.get('access_token') and len(res.json.get('access_token')) > 60)
                elif not failure:
                    self.assert200(res)
            ...
            test_login_json(
                None, 'admin', email='admin@example.com', success=True)
            ...
            new_user = model.User(
                user_name='test', email='test@example.com', password=hash_pwd('test'), active=True)
    ...
    def test_views(self):
        with self.client as c:
            ...
            self.assert200(res)
            self.assertTrue(
                'Roky v Bílém domě' in str(res.data, encoding='utf-8'))

            res = c.get('/ebooks/61944')
            self.assert200(res)
            self.assertTrue('docx' in str(res.data))

            res = c.get('/api/download/86060')
            self.assert200(res)
